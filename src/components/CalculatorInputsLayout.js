      ...
      <div className={ classes['inputs-container'] }>
        {
          mainCalculatorInputSymbols.map((symbol, index) => {
            if (index >= 0 && index <= 2) {
              return <div className={ classes['grid-item'] } key={index}><CalculatorButton buttonSymbol={symbol} additionalCSSType='utility' btnOnClick= { props.btnOnClick }/></div>
            } else if (symbol === 0) {
              return <div className={ classes['grid-item-span-two'] }  key={index}><CalculatorButton buttonSymbol={symbol} additionalCSSType='numbers' btnOnClick= { props.btnOnClick }/></div>
            } else if (['÷', 'x', '-', '+', '='].includes(symbol)) {
              return <div className={ classes['grid-item'] } key={index} ><CalculatorButton buttonSymbol={symbol} additionalCSSType='ops' btnOnClick= { props.btnOnClick }/></div>
            } else {
              return <div className={ classes['grid-item'] } key={index}><CalculatorButton buttonSymbol={symbol} additionalCSSType='numbers' btnOnClick= { props.btnOnClick }/></div>
            }
          })
        }
